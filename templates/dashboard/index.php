    ...
    <div class="bg-white p-6 rounded-xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-lg font-bold text-slate-800 tracking-tight">
                    Suivi Clinique des Lots Restants
                </h2>
            </div>
            <p class="text-xs text-slate-400 mt-0.5">
                Rapport de stock ordonné selon le protocole de rotation <span class="font-semibold text-slate-600">FEFO</span>
            </p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2 w-full md:w-auto text-xs">
            <?php if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
                <a href="index.php?action=users" class="px-3.5 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-lg font-semibold transition-all flex items-center gap-1.5 shadow-xs">
    ...
    <?php if (!empty($notifications)): ?>
        <div class="bg-amber-50/40 border border-amber-200 rounded-xl p-5 relative overflow-hidden">
            <div class="flex items-center gap-1.5 font-bold text-amber-800 text-[11px] tracking-wide uppercase mb-3.5">
                ...
                Péremptions Prévues (Mois Prochain)
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                <?php foreach ($notifications as $notif): ?>
                    <div class="bg-white border border-amber-100 p-3 rounded-lg flex justify-between items-center shadow-xs group">
                        <div>
                            <span class="font-semibold text-xs text-slate-700 block"><?= htmlspecialchars($notif['produit_nom']) ?></span>
                            <span class="inline-block text-[10px] text-slate-400 font-mono mt-0.5">Lot: <?= htmlspecialchars($notif['numero_lot']) ?></span>
                        </div>
                        <div class="text-right shrink-0">
                            <span class="font-mono font-bold text-amber-700 text-xs block"><?= $notif['date_peremption'] ?></span>
                            <span class="text-[9px] text-amber-800 bg-amber-100/60 px-1.5 py-0.5 rounded-full font-semibold uppercase mt-1 inline-block">Proche</span>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        <div class="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div class="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                Registre Général Virtuel
            </div>
            <span class="text-xs text-slate-500 bg-white px-2.5 py-1 rounded-md border border-slate-200 font-medium">
                Volume actuel : <span class="font-bold text-slate-800"><?= count($processedLots) ?></span> lot(s)
            </span>
        </div>
            ...
            <table class="w-full text-left border-collapse whitespace-nowrap text-xs">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-200 text-slate-400 font-semibold tracking-wide uppercase">
                        <th class="py-3 px-5">Médicament / Produit</th>
                        <th class="py-3 px-4">Référence</th>
                        <th class="py-3 px-4">N° de Lot</th>
                        <th class="py-3 px-4">Quantité</th>
                        <th class="py-3 px-4">Date de Péremption</th>
                        <th class="py-3 px-4">Statut Sanitaire</th>
                        ...
                        <?php foreach ($processedLots as $l): ?>
                            <tr class="hover:bg-slate-50/50 transition-colors duration-100">
                                <td class="py-3.5 px-5">
                                    ...
                                    <span class="text-[10px] text-slate-400 mt-0.5 block">Stock_ID: #<?= $l['id'] ?></span>
                                </td>
                                ...
                                <td class="py-3.5 px-4">
                                    <?php if(strpos($l['badge_text'], 'Rouge') !== false): ?>
                                ...
                                <?php if (isset($_SESSION['user_role']) && in_array($_SESSION['user_role'], ['pharmacien', 'admin'])): ?>
                                    <td class="py-3.5 px-5 text-right">
                                        <a href="index.php?action=expire&id=<?= $l['id'] ?>" 
                                           onclick="return confirm('Voulez-vous vraiment déclarer ce lot comme EXPIRED ?')"
                                           class="inline-flex items-center gap-1 bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-200 text-rose-600 font-bold text-[10px] uppercase tracking-wider px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer shadow-xs">
                                            Destruction
                                        </a>
                                    </td>
                                <?php endif; ?>
